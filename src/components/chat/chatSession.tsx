...
import { ChatMessage, InspectChats, Message,Connect} from "@/lib/types"
...
export default function ChatSession() {
  ...
  useEffect(() => {
    ...
    //chatHandler.websocket.send(JSON.stringify(connect_msg))

    chatHandler.websocket.onmessage = (event) => {
    ...
    <div className='w-full'>
      ...
      <div className='flex flex-col gap-10'>
        {messages.map((message, index) => (
          <MessageDisplay key={index} message={JSON.parse(message)}/>
        ))}
      </div >
